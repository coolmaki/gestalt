...
Bootstrap the pnpm workspace and scaffold the `@gestalt/core` package — the single shared frontend platform for all Gestalt apps. By the end of this plan, `pnpm build` compiles an empty library successfully.

## Design Decisions

| Decision | Rationale |
|----------|-----------|
| pnpm workspaces | AGENTS.md mandates pnpm. Workspaces enable `web/*` packages to reference each other as `@gestalt/core` without npm link. |
| Vite library mode | Outputs ESM + TypeScript declarations. All consumers are Vite-based SolidJS apps — no CJS consumer exists. |
| Single package `@gestalt/core` | One version, one dependency per app. Scope: design + components + PWA + auth + nav + i18n. |
...
| `web/@gestalt/core/` path | Namespaced under `@gestalt/` for shared packages. Apps at top level: `web/passport/`. |
...
4. [ ] Create `web/@gestalt/core/` directory structure:
   ```
   web/@gestalt/core/
   ├── src/
   │   ├── index.ts
   ...
   └── vite.config.ts
   ```
5. [ ] Write `package.json` — name `@gestalt/core`, type `module`, peer deps on `solid-js`, dev deps on `vite`, `typescript`, `@tailwindcss/vite`, `vite-plugin-solid`, `vite-plugin-dts`, `vitest`, `@solidjs/testing-library`
6. [ ] Write `tsconfig.json` — `module: ESNext`, `moduleResolution: bundler`, `jsx: preserve`, `jsxImportSource: solid-js`, strict, paths for `@gestalt/core` → `./src`
...
11. [ ] Run `pnpm build` from `web/@gestalt/core/` — verify zero errors, dist output exists

## Acceptance Criteria

- [ ] `pnpm-workspace.yaml` at repo root with `web/*` and `web/@gestalt/*`
- [ ] `.npmrc` with `shamefully-hoist=true`
- [ ] `web/@gestalt/core/package.json` with correct name and peer deps
